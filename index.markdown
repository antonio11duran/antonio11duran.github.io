---
# Feel free to add content and custom Front Matter to this file.
# To modify the layout, see https://jekyllrb.com/docs/themes/#overriding-theme-defaults

layout: home
---

## Hi, I'm Antonio 👋

I'm a Computer Science student at CU Boulder (B.S., May 2027) with a background in full-stack web and mobile development. I'm fluent in English and Spanish, and I bring a mix of technical skills and real-world experience in program coordination and team leadership.

**Languages:** Python, JavaScript, C++, Java, Scala, Kotlin, HTML5, CSS3  
**Frameworks:** React.js, Node.js, Express.js, Flask  
**Tools:** Git, GitHub, Android Studio, REST APIs, CI/CD

📄 [Resume](assets/Antonio_Duran_Resume.pdf) &nbsp;|&nbsp; 💻 [GitHub](https://github.com/antonio11duran) &nbsp;|&nbsp; ✉️ [Email](mailto:antonio11duran@gmail.com)
